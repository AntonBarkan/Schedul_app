...
class UsersController < ApplicationController
  ...
  def getPartDayArray(day)
    arr = Hash.new
    submitedHour = SubmitedHour.find_by(:week_start_date =>  Date.parse(day).to_s, :user_id => current_user.id)
    if submitedHour.nil?
      7.times do |index|
  ...
  def create_events(parsed_string, day)
    ...
    hours = SubmitedHour.find_by(:week_start_date =>  Date.parse(day).to_s, :user_id => current_user.id)
    if (hours.nil?)
      hours = SubmitedHour.create!
      hours.user_id =  current_user.id
      hours.week_start_date= Date.parse(day).to_s
    end

    puts '-------------------'
    puts "in create #{Date.parse(day)}  #{  hours.week_start_date}"
    puts '-------------------'
